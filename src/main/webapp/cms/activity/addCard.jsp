...
<body>
<div class="location">
	您当前的位置—通用活动-添加幻灯片 <span style="color: #F00">${message }</span>
</div>
<div class="checked">
	<form action="saveOrUpdateActivity.do" method="post" enctype="multipart/form-data" onsubmit="return check()">
	<table>
		<tr>
			<td>标题：</td>
			<td>
		...
		<tr>
			...
			<td>
				...
				<input type="hidden" id="img" name="img1" value="${card.img }" />
		...
		<tr>
			<td>链接：</td>
			<td>
				<input type="text" id="url" name="url" value="${card.url }" />
			</td>
		</tr>
		<tr>
			<td>排序：</td>
			<td>
				<input type="text" id="sorting" name="sort" value="${card.sort }" />
