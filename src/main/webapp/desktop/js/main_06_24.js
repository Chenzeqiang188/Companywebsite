...
$(function(){
	...
	/*(function(){
		var iNow=0;
		var oTimer=null;
		$('.dt_tab_dian li').click(function(){
			iNow=$(this).index();
			oTab(iNow);
		});
		
		$('.lk_dt_right').click(function(){
			oRightList();
		});
		$('.lk_dt_left').click(function(){
			oLeftlist();
		});
		$('.dt_tablist ul li').hover(function(){
			clearInterval(oTimer);	
		},function(){
			oTimer=setInterval(function(){
				oRightList();	
		...
		function oTab(){
			$('.dt_tab_dian li').eq(iNow).addClass('dt_active').siblings().removeClass();
			$('.dt_tablist ul li').hide().eq(iNow).show();
			$('.dt_gamejj p').hide().eq(iNow).show();	
		}
	})();*/
				...
				function animate(obj){
					obj.each(function(event){
						...
						if($(this).offset().top <= fold){
							...
							$(this).find('.gameleft').stop().animate({'left':'105px'},1000,"easeOutCubic");
								console.log(0);
							$(this).find('.game_peo').stop().animate({'right':'105px'},1000,"easeOutCubic");
